...
def dibujar_calendario(agenda: pd.DataFrame, inicio_bloque: str, fin_bloque: str):
    ...
    hora_ini = int(inicio_bloque.split(":")[0])
    hora_fin = int(fin_bloque.split(":")[0])
    ...
    for qx in qx_orden:
        ...
        for _, row in agenda_qx.iterrows():
            ...
            left = (offset / total_min) * 100
            width = min(100 - left, (dur / total_min) * 100)

            proc = truncar(row["procedimiento_base"] if "procedimiento_base" in row else row["procedimiento"], 34)
...
def preparar_tabla_candidatos(candidatos: pd.DataFrame) -> pd.DataFrame:
    ...
    return tabla[[
        "Procedimiento", "Quirófano", "Inicio", "Fin estimado",
        "Necesaria", "Disponible", "Holgura", "Habitual"
    ]]


def render_tabla_candidatos(candidatos: pd.DataFrame):
    st.markdown("<div class='panel-card' style='min-height:auto; margin-top:18px;'>", unsafe_allow_html=True)
...
if anadir:
    if st.session_state.candidatos.empty:
        ...
        st.session_state.agenda_actual = pd.concat(
            [st.session_state.agenda_actual, pd.DataFrame([nueva])],
            ignore_index=True,
